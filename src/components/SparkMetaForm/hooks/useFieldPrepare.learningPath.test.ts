...
describe('LP root fallback fields (no category-definition form config yet)', () => {
  it('renders just policy for an LP root instead of the Course BMGS skeleton — no Curriculum/framework field at all', () => {
...
describe('LP root has no Curriculum section (removed — learning_path_multi_framework_skills_plan.md §1)', () => {
  ...
  it('never renders framework/category fields for an LP root, whatever framework happens to be resolved', () => {
    const fields = useFieldPrepare([], {}, usf, true, { profile: learningPathProfile });
    expect(fields.map(f => f.code)).toEqual(['name', 'description', 'keywords', 'policy']);
  });

  it('passes a formConfig-driven LP root form through untouched — no field dropped, none synthesized', () => {
    const cfg = [
      { code: 'name', label: 'Name', inputType: 'text' },
      { code: 'policy', label: 'Consumption policy', inputType: 'select', range: ['strict', 'adaptive', 'priorLearning'] },
    ];
    const fields = useFieldPrepare(cfg, {}, usf, true, { profile: learningPathProfile });
    expect(fields.map(f => f.code)).toEqual(['name', 'policy']);
  });
});
